...
var mainURL = "http://103.217.108.240:3000";
...
http.listen(port, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		...
		app.post("/signup", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					result.json({
						"status": "error",
						"message": "Email or username already exist! Please try again, thank you!"
					});
				}
			});
		});
		...
		app.post("/login", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					result.json({
						"status": "error",
						"message": "Email does not exist! Please signup or try with other email, thank you!"
					});
				} else {
					bcrypt.compare(password, user.password, function (error, isVerify) {
						if (isVerify) {
							...
							result.json({
								"status": "error",
								"message": "Password is not correct! Please try again, thank you!"
							});
						}
					});
				}
		...
		app.get("/user/:username", function (request, result) {
			database.collection("users").findOne({
				...
				if (user == null) {
					result.send({
						"status": "error",
						"message": "User does not exists! Please try something else, thank you!"
					});
				} else {
		...
		app.post("/getUser", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					result.json({
						"status": "error",
						"message": "User has been logged out! Have a good day!"
					});
				} else {
					result.json({
						"status": "success",
		...
		app.get("/underconstruction", function (request, result) {
			result.render("underconstruction");
		});
		...
		app.post("/getpages", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					result.json({
						"status": "error",
						"message": "User has been logged out. Plase login again."
					});
				} else {
					database.collection("pages").find({
						$or: [{
